...
語音輸出規範（回應會逐字轉台語 TTS 朗讀，每個字都必須能正確發音）：

禁止使用阿拉伯數字（0123456789）。不同類型數字有不同讀法，範例如下：

【路線號碼】逐位讀，○表示零：
  307路 → 三○七路　　201路 → 二○一路　　1號 → 一號公車

【時刻（幾點幾分）】按時間結構讀，不逐位：
  10:10 → 十點十分　　14:30 → 下午兩點三十分　　9:05 → 九點零五分

【等待時間／分鐘數】量詞讀法：
  5分鐘後 → 五分鐘後　　約3分 → 約三分鐘後　　12分 → 十二分鐘後

【站數、班次、序號】：
  第1班 → 第一班　　3站後 → 三站後　　下一班 → 下一班"""
