...
`configure_telemetry()` 在 root logger（`logging.getLogger()`）掛一個
`LoggingHandler`，把整個程式碼庫既有的 `_log = logging.getLogger(__name__)`
呼叫（`providers/`、`services/`、`voice/` 等散落各處）原封不動導去 OTLP，
不需要在呼叫端改任何程式碼。

- Handler 掛在 `level=logging.NOTSET`，實際過濾靠各 logger 自己的
  effective level——目前沒有任何 `basicConfig`/`setLevel`，root 預設是
  `WARNING`，所以現況是 WARNING 以上（`_log.warning` / `_log.error`）才會
  送出，跟目前終端機看到的輸出一致。
- SigNoz 對應表：`signoz_logs.logs_v2`，UI 的 **Logs** 頁面用
  `service.name = taigi-bus-agent` 篩。
- **已知限制（ponytail）**：uvicorn 的 `uvicorn.access` / `uvicorn.error`
  logger 預設 `propagate=False`，不會傳到 root，所以 HTTP access log 目前
  不會進 SigNoz——只有這個專案自己模組的 log 會。access log 之後真的需要
  再另外接 uvicorn 的 log config。
- 內容政策跟既有 diagnostic span event 一致：這些呼叫點本身就只寫運維訊息
  （上游失敗、fallback、逾時），不含 user input / prompt。
...
http://localhost:8080。細節見 `backend/telemetry/README.md`。

相關設定都在 `backend/telemetry.py`（backend 根層級的中立 infra 模組，
agent / api / providers / services 都可引用，不構成跨層依賴）：
- `configure_telemetry()`：初始化 TracerProvider + MeterProvider + LoggerProvider，加 OTLP exporter 與 bucket view
